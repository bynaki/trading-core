...
이 예제 전용 `setting.toml`로 로그 모듈을 구성하고, 피드를 구독한 뒤 `shutdown()`으로 큐를
비운다. 마지막에 JSON 로그 파일을 **이번 실행의 발신처(instance_id)로 걸러** 읽어 보인다.
여러 서버의 로그가 모인 로그서버에서 한 프로세스의 로그만 골라 보는 것과 같은 방식이다.
"""
...
from trading_core.logger import configure, get_logger, load_settings, shutdown
...
def report(path: Path, instance_id: str) -> None:
    ...
    print(f"\n----- {path} 에서 instance_id={instance_id} 인 레코드 {len(records)}건 -----")
    print("레벨별:", dict(Counter(r["level"] for r in records)))
    print("로거별:", dict(Counter(r["logger"] for r in records)))
    print("태스크별:", dict(Counter(r["task"] for r in records)))

    failed = next(r for r in records if r["exc"] is not None)
    failed["exc"]["traceback"] = failed["exc"]["traceback"].splitlines()[-1]  # 보기 좋게 줄인다
    print("\n예외를 담은 레코드 하나 (traceback은 마지막 줄만):")
...
async def run_ex(domain: Domain) -> None:
    ...
    log.info("예제 끝", received=received)

    shutdown()  # 큐에 남은 레코드를 모두 내보낸다. 파일을 읽기 전에 불러야 한다
