...
확인하려는 불변식은 하나다. **파생 스테이지가 상위 원천에 등록하는 심볼은 구독자
전체의 합집합이어야 한다.** 각 콜백은 자신이 어떤 심볼로 (재)시작했는지 출력하므로,
`[DEPENDENT]` 줄과 `[ORIGIN]` 줄의 심볼을 비교하면 두 계층의 구독 상태가 맞물려
있는지 눈으로 확인할 수 있다. 이 불변식이 깨지면 먼저 구독한 쪽이 아무 오류 없이
조용히 굶는다 — 자세한 내력은 `README.md`에 있다.
"""
...
async def _(ctx: PriceRequest, symbols: set[str], recv: Receiver):
    ...
    `[ORIGIN]` 줄의 심볼을 비교해 보면 두 계층의 구독 상태가 어긋나는 것이 보인다.
    """
