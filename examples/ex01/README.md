...
2. `@generator(CountReq)`가 `CountReq`를 처리할 제너레이터를 등록한다.
3. `gen01()`은 요청 모델 자체를 스테이지 실행 컨텍스트로 사용한다.
4. `@gen01.bind` 함수가 구독 심볼을 순환하며 `CountData`를 계속 발행한다.
...
- `bind` 함수의 `finally`: 심볼 구성이 바뀌어 현재 업데이트가 재시작되거나 요청
  소비가 중단될 때 실행된다. 업데이트 단위의 네트워크 연결·구독 같은 자원을
  정리하기 적합하다.
- `@gen01.close`: 해당 요청의 마지막 구독자가 사라져 원천 스테이지가 제거될 때
...
- 현재 binder는 `count`가 0이 아닌 동안 계속 증가하므로 스스로 끝나지 않는다.
  소비자가 `break`하거나 태스크가 취소되어야 종료된다.
- `start=0`이면 `while count` 조건이 처음부터 거짓이므로 데이터가 발행되지 않는다.
- 심볼은 `set`으로 전달되므로 최초 순서는 보장되지 않는다. 다만 실행 중 만들어진
  심볼 목록 안에서는 카운트가 증가할 때마다 순환한다.
- 빈 심볼 집합은 발행 대상을 만들 수 없으므로 사용하지 않는다.
