...
binder가 받은 데이터의 심볼이 현재 구독 심볼 집합에 없으면 예외 대신 경고 로그
(`ex04.ohlc`)만 남기고 건너뛴다. 변환이 어긋난 상황을 스트림을 끊지 않고 드러내기 위한 방어 코드다.
...
- `run_ex.py`는 앞 요청의 구독을 모두 정리한 뒤 다음 요청을 시작한다. 따라서 원천
  태스크도 하나씩만 살아 있다. `TaskManager`의 태스크 로그는 DEBUG라 기본 예제 설정에서는
  보이지 않는다. `examples/setting.toml`의 두 `level`을 `"DEBUG"`로 바꾸면 보인다.
