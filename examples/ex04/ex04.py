...
`require` 콜백이 상위 요청과 상위 심볼 집합을 함께 반환하며, 그 두 반환값이 각각
다음을 결정한다.

1. **어느 원천에 붙을지** — `OHLCRequest.quote`에 따라 `BinanceRequest`(USD)와
   `UpbitRequest`(KRW) 중 하나를 반환하므로 상위 원천 스테이지 자체가 달라진다.
2. **어떤 심볼로 구독할지** — 소비자는 `"BTC"` 같은 기초 자산만 요청하지만, 원천에는
   `"BTC/USD"` · `"BTC/KRW"`처럼 그 거래소의 표기로 바뀌어 등록된다.

역방향 변환은 파생 binder가 맡는다. `base_of()`로 심볼을 기초 자산으로 되돌려야
구독자에게 데이터가 닿는다 — 자세한 내용은 `README.md`에 있다.
"""
...
def ohlc_requirement(req: OHLCRequest, symbols: set[str]):
    ...
    반환하는 두 값이 각각 상위 원천과 상위 심볼을 정한다.

    - 첫 번째 값(요청): `quote`에 따라 타입 자체가 달라지므로 하나의 파생 요청이 두
      원천 중 하나로 라우팅된다. 분기 기준이 **요청 필드**라는 점이 중요하다. 심볼에
      따라 다른 상위 요청을 반환하면 안 된다 — 파생 스테이지는 상위를 하나만 본다.
    - 두 번째 값(심볼): 심볼 집합까지 받는 형태의 콜백이라 상위 원천에는 `"BTC"`가
      아니라 `"BTC/USD"` · `"BTC/KRW"`가 등록된다. 넘어오는 `symbols`는 그 시점
      구독자 전체의 합집합이다.
    """
...
async def _(ctx: OHLCRequest, symbols: set[str], recv: Receiver):
    ...
    require가 한 변환을 여기서 되돌린다. 모델과 심볼 둘 다 되돌려야 한다.
