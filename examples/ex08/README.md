# ex08: 같은 `content_id`라도 `SessionRequest`는 공유되지 않는다

이 예제가 보여 주는 것은 이 한 문장이다.

> `SessionRequest`는 두 요청이 같은 내용의 요청이라도(같은 content_id) 서로 다른 요청으로
> 취급하며 컨텍스트를 공유하지 않는다.

ex01~ex06에서 "content_id가 같으면 원천 하나를 공유한다"는 것은 프레임워크의 1번
불변식이었다. 필드 값이 같은 요청을 두 소비자가 보내면 `Domain`은
`_shared_stages[content_id]`에 있는 스테이지를 재사용하고, init 콜백은 한 번만
불리며, 컨텍스트와 generator를 함께 쓴다.

`SessionRequest`(세션 요청)에는 그 규칙이 적용되지 않는다. **요청을 보낸 수만큼 스테이지가
...
| --- | --- | --- |
| 세션 | `WatchReq(quote="USD")` | 스테이지 **둘**, 컨텍스트 **둘**, `"BTC"` 슬롯 **둘** |
| 원천 | `BeatReq()` | 스테이지 **하나**, 컨텍스트 **하나**, generator **하나** |

두 소비자의 세션 스테이지가 각각 상위 `BeatReq`를 구독하는데, 그 상위는 여전히
content_id 단위로 공유된다. 공유가 사라진 것이 아니라 **공유의 경계가 세션 스테이지
아래로 내려간 것**이다.

## 파일 구성

- `ex08.py`: 원천 `BeatReq`와 세션 요청 `WatchReq` 한 쌍. 양쪽 init 콜백이 자기가 몇 번째로
  불렸는지 로그로 남긴다. 원천은 `ex08.beat`, 세션 요청은 `ex08.watch` 로거다.
...
3. `WatchCtx`가 **두 번** 만들어지고 bind 콜백도 **두 번** 불린다. 같은 content_id, 같은
   심볼인데도 그렇다.
4. 두 파이프라인은 모두 상위 표기 `"BTC/USD"`로 `BeatReq`를 구독한다. 상위는 content_id
...
```text
ex08: ━━━━━━━━━━ 시작: 같은 content_id라도 SessionRequest는 공유되지 않는다 ━━━━━━━━━━
...
뜻이다. 스테이지별 상태(구독 파이프라인 번호, 인증 토큰, 재연결 카운터 같은 것)를 컨텍스트에
두려 할 때 알아 두어야 하는 성질이다.

**`BeatCtx 생성`은 한 번뿐이다.** 세션 요청이 공유되지 않는다고 해서 그 위의 원천까지
복제되지는 않는다. 두 스테이지가 각각 만든 파이프라인이 모두 `"BTC/USD"`를 요구하고, 그
...
세션 요청은 전제가 다르다. 슬롯이 심볼 단위이고 각 슬롯이 자기 `Runnable` 상태를 들고
있다(ex07의 `SwingRunnable.active_price`). 두 소비자가 같은 심볼을 구독했더라도 그
상태를 공유하는 것이 옳다는 보장이 없다. 그래서 세션 요청은 공유를 아래 계층에 맡기고,
자신은 요청마다 독립적으로 선다.

## 알아둘 점

- 컨텍스트를 정말 공유해야 한다면 `WatchCtx`가 아니라 그 아래 원천 요청의 컨텍스트에
  두어야 한다. 세션 컨텍스트는 소비자별 상태를 두는 자리다.
- 세션 스테이지가 늘어난다고 상위 연결이 늘어나지는 않는다. 늘어나는 것은 슬롯과
  라우팅 대상이고, 상위 generator 수는 상위 표기 합집합이 정한다.
- 소비자가 많아질수록 세션 스테이지도 그만큼 생긴다. 슬롯마다 `Channel`과 태스크가
...
`_shared_stages`에 넣는 줄도 없다. `run_ex.py`의 `report_sharing()`이 그것을 그대로
확인한다 — `BeatReq`의 content_id로는 `get_shared_symbols()`가 구독 심볼을 돌려주지만,
`WatchReq`의 content_id로는 빈 집합이 나온다.
