# ex07: `SessionRequest`로 심볼마다 파이프라인 붙이기

ex01~ex06의 요청은 모두 "심볼 집합 하나 → generator 하나"였다. `SourceRequest`든
`DerivedRequest`든, binder는 심볼 **집합**을 통째로 받아 하나의 async generator를
돌렸다.

`SessionRequest`는 다르다. **심볼마다 슬롯 하나**를 만들고, 각 슬롯이 `Pipeline`으로 상위
원천에 붙는다. 등록되면 모델 타입이 `"session"`이 된다.

이 예제가 보여 주는 것은 이 한 문장이다.

> 소비자가 구독하는 심볼과 원천이 아는 심볼은 달라도 된다. 그 변환을 파이프라인이 맡는다.

## 상위 표기와 하위 표기

세션 요청을 이해하려면 두 심볼을 구분해야 한다. 이름이 헷갈리면 나머지가 다 헷갈린다.
...
- `ex07.py`: 원천(`TickReq`)과 세션 요청(`SwingReq`) 한 쌍, 그리고 그 사이를 잇는
  `SwingRunnable`.
- `run_ex.py`: `Domain.stream()`으로 스윙 6건을 받고 끝낸다.

## 예제의 흐름

1. `TickReq`는 필드가 없는 원천이다. 상위 표기로 현재가를 0.5초마다 1씩 올려 발행한다.
2. `SwingReq(quote="USD")`를 `{"BTC", "ETH", "XRP"}`로 구독한다.
3. 심볼마다 bind 콜백이 한 번씩 불려 파이프라인을 하나씩 만든다. 세 슬롯이 생긴다.
4. 각 파이프라인은 `TickReq()("BTC/USD")`로 상위를 구독하고 `| SwingRunnable("USD", "BTC")`로
   변동폭 계산 단계를 잇는다.
5. `Domain`은 세 파이프라인이 요구한 상위 표기의 합집합을 `TickReq` 원천에 등록한다.
...
```text
ex07: ━━━━━━━━━━ 시작: SessionRequest로 심볼마다 파이프라인 붙이기 ━━━━━━━━━━
...
파이프라인 단계가 `None`을 반환하면 그 데이터는 소비자에게 가지 않는다. 비교할 직전 값이
...
- `@x.always`를 쓰면 구독 심볼과 무관하게 늘 붙는 파이프라인을 하나 더 둘 수 있다(하트비트
  채널 같은 것). 이 예제는 쓰지 않는다.

## 어디를 검증하는가

`domain.py`의 `_create_session_subscription()`다. 특히 상위 스테이지에 등록할 심볼을 고르는 곳이
이 예제의 핵심이다.
...
`tests/test_domain.py`의 `test_session_stage_maps_symbols_to_the_upstream`이다.
