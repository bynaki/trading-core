...
class TaskManager:
    def __init__(self, id: str = ""):
        ...
        self._failure_cb: Callable[[Exception, str], Awaitable[None]] | None = None
    ...
    async def submit(self, coro: Coroutine[Any, Any, None], name: str):
        ...
        self._live_count += 1
        log.debug("태스크 제출", id=self._id, name=name, live_count=self._live_count)
    ...
    @property
    def live_count(self):
        """제출된 뒤 아직 끝나지 않은(대기 + 실행) 태스크 수."""
        return self._live_count

    def set_failure_callback(self, cb: Callable[[Exception, str], Awaitable[None]]):
        """태스크가 예외로 끝났을 때 부를 콜백을 등록한다."""
        self._failure_cb = cb
    ...
    def _skip_pending(self, coro: Coroutine[Any, Any, None], name: str):
        ...
        self._live_count -= 1  # _task_wrapper를 거치지 않으므로 직접 차감
        self.on_task_cancelled(name)
        self._release(name)
    ...
    def on_task_finished(self, name: str):
        log.debug("태스크 완료", id=self._id, name=name, live_count=self._live_count)

    def on_task_cancelled(self, name: str):
        log.debug("태스크 취소", id=self._id, name=name, live_count=self._live_count)
